                                    ...
                                    <div class="dropdown-menu dropdown-menu-left px-2 text-right mt-2">
                                        ...
                                        <div class="pt-4 pb-1 border-t border-gray-200">
                                            ...
                                            <div class="mt-3 space-y-1">
                                                <!-- Account Management -->
                                                <responsive-nav-link href="{{ route('profile.show') }}" :active="request()->routeIs('profile.show')">
                                                    {{ __('الملف الشخصي') }}
                                                </responsive-nav-link>

                                                @if (Laravel\Jetstream\Jetstream::hasApiFeatures())
                                                    <responsive-nav-link href="{{ route('api-tokens.index') }}" :active="request()->routeIs('api-tokens.index')">
                                                        {{ __('API Tokens') }}
                                                    </responsive-nav-link>
                                                @endif
                                                ...
                                                @if (Laravel\Jetstream\Jetstream::hasTeamFeatures())
                                                    ...
                                                    <!-- Team Settings -->
                                                    <responsive-nav-link href="{{ route('teams.show', Auth::user()->currentTeam->id) }}" :active="request()->routeIs('teams.show')">
                                                        {{ __('Team Settings') }}
                                                    </responsive-nav-link>

                                                    @can('create', Laravel\Jetstream\Jetstream::newTeamModel())
                                                        <responsive-nav-link href="{{ route('teams.create') }}" :active="request()->routeIs('teams.create')">
                                                            {{ __('Create New Team') }}
                                                        </responsive-nav-link>
                                                    @endcan
                                                    ...
                                                    @foreach (Auth::user()->allTeams() as $team)
                                                        <switchable-team :team="$team" component="jet-responsive-nav-link" />
                                                    @endforeach
                                                @endif
                                            </div>
                                        </div>
